...
- 2026-08-16：**清理脚手架遗留组件**（用户确认删除）——HomeView.vue（步骤 1 占位页，无路由引用）与 HelloWorld.vue（Vite 脚手架示例）删除，路由 `/` 由 ShelfView 承载；同步 frontend/doc/README.md
- 2026-08-16：**单词英语音标（步骤 17，迭代②）**——AI 生成 prompt words 增加 `phonetic`（IPA，`/ /` 包裹；校验宽松缺失不拒，仅新生成带出）；FavoriteItem + words（收藏列表带出标注单词 JSON）、VocabItem + phonetic（生词列表匹配单词时带出）；前端四处展示：SentenceBubble 单词列表项、WordBubble 头部、VocabView 生词卡片、FavoritesView **可收起单词列表**（默认收起，含音标/词性/意思，同讲解弹窗样式）；无 phonetic 不显示不占位。**真实 API 验证**：新生成 9/9 词带音标（/ˈbɪli/ /ˈæpəlz/ 等）、收藏/生词列表带出、存量标注 None 正常降级；vue-tsc + build 通过
