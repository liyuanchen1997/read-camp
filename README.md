...
- **双语精读**：英文/中文左右对照（PC）、逐句同步滚动、双向悬停高亮；文章分章节展示 + 目录（桌面侧栏/移动端折叠）+ 上一章/下一章；整篇中文翻译一键开关
- **句级/词级讲解**：点击句子弹出气泡 —— 句子解释、中文意思、句子成分、每个单词的词性与作用（含 IPA 音标）；单词可单独查看、发音、加入生词本
- **发音**：整篇顺序朗读（当前句高亮跟随）、单句/单词发音，浏览器原生 TTS，免费
- **学习数据**：生词本（卡片含音标/词性/释义）、例句收藏（可展开单词列表含音标）、近期阅读（进度条）、精读文章数与总进度统计
...
**第一版开发完成**：计划 17 个步骤全部交付（核心功能 + 迭代①文章分章节 + 迭代②单词音标），进度与验收记录见 [doc/02-project-plan.md](doc/02-project-plan.md)。
